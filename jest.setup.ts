...
jest.mock('@/stores', () => {
  const makeStoreMock = <T>(state: T) =>
    Object.assign(
      jest.fn((selector?: (s: T) => unknown) => (selector ? selector(state) : state)),
      { getState: () => state }
    );
  ...
  return {
    ...
    useCapacityStore: makeStoreMock({
      capacities: {},
      children: {},
      language: 'en',
      timestamp: 0,
