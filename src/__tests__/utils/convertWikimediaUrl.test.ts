import { 
  convertToCommonsPageUrl, 
  ensureCommonsPageUrl 
} from '@/lib/utils/convertWikimediaUrl';
...
describe('convertWikimediaUrl', () => {
  describe('convertToCommonsPageUrl', () => {
    it('should convert direct file URL to Commons page URL', () => {
      const directUrl = 'https://upload.wikimedia.org/wikipedia/commons/e/e1/Wikipedia_de_A_a_Z.pdf';
      const result = convertToCommonsPageUrl(directUrl);
      
      expect(result.isWikimediaUrl).toBe(true);
      expect(result.commonsPageUrl).toBe('https://commons.wikimedia.org/wiki/File:Wikipedia_de_A_a_Z.pdf');
    ...
    it('should handle URLs with special characters', () => {
      const urlWithSpaces = 'https://upload.wikimedia.org/wikipedia/commons/a/ab/File_with_spaces.pdf';
      const result = convertToCommonsPageUrl(urlWithSpaces);
      
      expect(result.commonsPageUrl).toBe('https://commons.wikimedia.org/wiki/File:File_with_spaces.pdf');
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle URLs with query parameters', () => {
      const urlWithParams = 'https://upload.wikimedia.org/wikipedia/commons/e/e1/Wikipedia_de_A_a_Z.pdf?version=1';
      const result = convertToCommonsPageUrl(urlWithParams);
      
      expect(result.commonsPageUrl).toBe('https://commons.wikimedia.org/wiki/File:Wikipedia_de_A_a_Z.pdf?version=1');
    });
  });
