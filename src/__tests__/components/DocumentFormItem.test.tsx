...
const mockPageContent = {
  ...
  'snackbar-edit-profile-organization-invalid-document-url': 'Invalid document URL. Please use a Wikimedia Commons URL (commons.wikimedia.org) with less than 200 characters.',
  'snackbar-edit-profile-organization-document-url-too-long': 'Document URL is too long. Please use a URL with less than 200 characters.',
  'snackbar-edit-profile-organization-document-url-invalid-format': 'Invalid URL format. Please use a valid Wikimedia Commons URL.',
};
...
describe('DocumentFormItem', () => {
  ...
  it('calls onChange when input value changes', () => {
    ...
    renderWithProviders(
      <DocumentFormItem {...defaultProps} onChange={mockOnChange} />
    );
  ...
  it('calls onDelete when delete button is clicked', () => {
    ...
    renderWithProviders(
      <DocumentFormItem {...defaultProps} onDelete={mockOnDelete} />
    );
  ...
  it('handles empty document url', () => {
    ...
    renderWithProviders(
      <DocumentFormItem {...defaultProps} document={emptyDocument} />
    );
  ...
  it('handles null url gracefully', () => {
    ...
    renderWithProviders(
      <DocumentFormItem {...defaultProps} document={documentWithNullUrl} />
    );
  ...
  it('shows validation checkmark for valid Commons URLs', async () => {
    const validDocument = { id: 1, url: 'https://commons.wikimedia.org/wiki/File:Test.pdf' };
    renderWithProviders(
      <DocumentFormItem {...defaultProps} document={validDocument} />
    );
  ...
  it('shows error indicator for invalid URLs', async () => {
    ...
    renderWithProviders(
      <DocumentFormItem {...defaultProps} document={invalidDocument} />
    );
  ...
  it('does not show snackbar for valid URLs', async () => {
    ...
    const input = screen.getByDisplayValue('https://example.com/document');
    fireEvent.change(input, { target: { value: 'https://commons.wikimedia.org/wiki/File:Valid.pdf' } });
