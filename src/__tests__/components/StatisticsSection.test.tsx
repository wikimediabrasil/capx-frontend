...
describe("StatisticsSection", () => {
  ...
  it("renders loading state correctly", () => {
    ...
    const loadingText = screen.getByText(
      mockTranslations["statistics-loading-text"]
    );
  ...
  it("renders error state correctly", () => {
    ...
    const errorText = screen.getByText(
      mockTranslations["statistics-error-text"]
    );
  ...
  it("renders all statistics cards with correct data", () => {
    ...
    // Check section title
    expect(
      screen.getByText(mockTranslations["statistics-section-title"])
    ).toBeInTheDocument();

    // Check card titles
    expect(
      screen.getByText(mockTranslations["statistics-users-title"])
    ).toBeInTheDocument();
    expect(
      screen.getByText(mockTranslations["statistics-capacities-title"])
    ).toBeInTheDocument();
    expect(
      screen.getByText(mockTranslations["statistics-messages-title"])
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByText(`+89 ${mockTranslations["statistics-new-label"]}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`+782 ${mockTranslations["statistics-new-label"]}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`+32 ${mockTranslations["statistics-new-label"]}`)
    ).toBeInTheDocument();
  });
  ...
  it("renders navigation links correctly", () => {
    ...
    const capacitiesLink = screen.getByText(
      mockTranslations["navbar-link-capacities"]
    );
    const organizationsLink = screen.getByText(
      mockTranslations["navbar-link-organizations"]
    );
  ...
  it("applies dark mode classes when darkMode is true", () => {
    ...
    const title = screen.getByText(
      mockTranslations["statistics-section-title"]
    );
  ...
  it("includes data source text", () => {
    ...
    const sourceText = screen.getByText(
      mockTranslations["statistics-source-text"]
    );
