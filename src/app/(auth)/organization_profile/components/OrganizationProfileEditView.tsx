      ...
      <div className="flex flex-col gap-4 md:hidden w-full mt-4">
        ...
        <div className="flex flex-row justify-between gap-4 items-center">
          ...
          <div className="relative w-[75px] h-[75px]">
            <Image
              src={userProfileImageUrl || getProfileImage(undefined, userProfile?.avatar, avatars)}
              alt="Avatar"
              fill
              sizes="75px"
              priority
...
function LogoInputSection({ darkMode, pageContent, formData, setFormData }: SectionShellProps) {
  return (
    <div className="mt-6">
      <div className="flex items-center gap-2 mb-4">
...
function ReportSection({ darkMode, pageContent, formData, setFormData }: SectionShellProps) {
  return (
    <div className="mt-6">
      <div className="flex items-center gap-2 mb-4">
...
const CAPACITY_FIELD_BY_TYPE: Record<string, string> = {
  known: 'known_capacities',
  available: 'available_capacities',
  wanted: 'wanted_capacities',
};

const CAPACITY_BORDER_CLASS_BY_TYPE: Record<string, string> = {
  known: 'border-[#0070B9]',
...
function CapacityChip({
  ...
  const displayName =
    ...
    (capacityName.startsWith('https://') || capacityName.includes('entity/Q'))
      ? `Capacity ${capacity}`
      : capacityName;
  const borderColorClass = CAPACITY_BORDER_CLASS_BY_TYPE[type];
...
function CapacitiesEditSection({
  darkMode,
  pageContent,
  formData,
  getCapacityName,
  handleRemoveCapacity,
  handleDirectCapacityAdd,
  handleImportKnownCapacities,
}: {
  darkMode: boolean;
  pageContent: any;
  formData: any;
  getCapacityName: any;
  handleRemoveCapacity: any;
  handleDirectCapacityAdd: any;
  handleImportKnownCapacities: any;
}) {
  const sections = [
    {
      type: 'known' as const,
  ...
  return (
    <div className="space-y-6 mt-8">
      {sections.map(({ type, icon, title, helpText, showImport }) => {
        const capacities = formData[CAPACITY_FIELD_BY_TYPE[type]] || [];
...
function TerritorySection({
  ...
  setFormData,
  territories,
}: SectionShellProps & { territories: any }) {
  return (
    <div className="mt-6">
      <div className="flex items-center gap-2 mb-4">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {formData.territory?.map((terr: any, index: number) => (
          <div
            key={terr + index}
          ...
          >
            <span className="font-[Montserrat] text-xs md:text-[24px]">{territories[terr]}</span>
            <button
              onClick={() => {
...
function EventsSection({
  ...
  handleEditEvent: any;
  handleChooseEvent: any;
  handleAddEvent: any;
  handleViewAllEvents: any;
}) {
  return (
    <div className="mt-6">
      <div className="flex items-center gap-2 mb-4">
...
function NewsSection({
  ...
  pageContent: any;
  diffTagsData: any;
  handleDeleteDiffTag: any;
  handleDiffTagChange: any;
  handleAddDiffTag: any;
}) {
  return (
    <div className="mt-6">
      <div className="flex items-center gap-2 mb-4">
      ...
      <div className="flex flex-col w-full gap-2 mb-2">
        {diffTagsData?.map((tag: any, index: number) => (
          <NewsFormItem
  ...
  return (
    <section className="w-full max-w-screen-xl py-8">
      ...
      <div className="flex flex-col gap-4">
        {contactFields.map(({ icon, field, placeholder }) => (
          <div
            key={field}
            className={`flex flex-row border-[1px] border-[solid] w-full px-1 lg:px-3 py-1 lg:py-4 md:px-[12px] md:py-[24px] items-center gap-3 md:gap-[12px] rounded-md md:rounded-[16px] ${
              darkMode ? 'bg-capx-dark-box-bg border-white' : 'bg-[#FFF] border-capx-dark-box-bg'
            }`}
          >
            <div className="relative w-8 h-8 md:w-[48px] md:h-[48px]">
              <Image
    ...
    >
      ...
      >
        <div className="flex flex-col gap-6 mx-auto w-full">
          ...
          <CapacitiesEditSection
            darkMode={darkMode}
            pageContent={pageContent}
            formData={formData}
            getCapacityName={getCapacityName}
            handleRemoveCapacity={handleRemoveCapacity}
            handleDirectCapacityAdd={handleDirectCapacityAdd}
            handleImportKnownCapacities={handleImportKnownCapacities}
          />
