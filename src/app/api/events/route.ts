...
// Build the backend payload from a raw event, keeping only valid fields
function buildCleanedEvent(event: any) {
  ...
  // related_skills is already set above with default empty array, but update if provided
...
// Map an event-creation error to the appropriate JSON response
function handlePostError(error: any): NextResponse {
...
export async function POST(request: NextRequest) {
  ...
  const cleanedEvent = buildCleanedEvent(event);
