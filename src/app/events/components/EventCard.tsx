...
export default function EventCard({ event }: EventCardProps) {
  ...
  return (
    <div className={`"flex flex-col gap-4 bg-[#EFEFEF]
    rounded-[4px] p-4" ${darkMode ? "text-white" : "bg-white text-dark-box-bg"}`}>
      <div className="flex gap-4 mx-4 my-4 w-full justify-between">
        <div className="flex flex-col gap-2">
          <h2 className="text-xl font-extrabold mb-2 text-dark-box-bg font-Montserrat">{event.name}</h2>
          
          {organization && (
            <p className="text-md mb-2">
              <span className="font-Montserrat">{pageContent["organization-profile-event-organized-by"] || "Organized by: "}</span> 
              <Link href={`/organization_profile/${organization.id}`} className="text-blue-600 hover:text-blue-800 visited:text-blue-800">{" "}{organization.display_name}</Link>
            </p>
          )}
          ...
          <div className={`flex gap-4 ${isMobile ? "flex-col" : "flex-row"}`}>
            ...
            <div className="flex flex-col gap-4 mb-2">
              {event.related_skills && (
                <div className="flex flex-row gap-2">
                  <Image src={darkMode ?  EmojiObjectsIcon :  EmojiObjectsDarkIcon}
                    width={24}
                    height={24}
                    alt="Emoji" />
                  <p className="text-md font-extrabold text-[#507380]">
              ...
              <div className="flex flex-row gap-2 justify-between">
                <div className="flex flex-row gap-2">
                  {event.related_skills.map((skill) => (
                    <p key={skill} className="text-sm px-2 py-1 rounded-[4px] bg-capx-dark-box-bg text-white rounded-[8px] w-fit">
                    {capacityNames[skill]}
                    </p>
                  ))}
                  {isMobile &&
                    <Image src={MoreHorizIcon} alt="More" />                    
                  }
                </div>
              </div>
              {
                isMobile &&
                <div className="flex flex-row gap-2 justify-between">
                  <Link className="flex flex-row gap-2" href={`/organization_profile/${event.organization}`}>
                    <p className="text-md font-extrabold text-[#507380]">
      ...
      <div className="flex flex-row gap-2 my-4 mx-4">
        <BaseButton customClass={`flex justify-center items-center gap-2 px-8 py-4 rounded-lg text-white font-extrabold rounded-lg bg-capx-dark-box-bg text-center not-italic leading-[normal] ${isMobile ? "text-md" : "text-lg"}`} label="Contact" />
        <BaseButton customClass={`flex justify-center items-center gap-2 px-8 py-4 rounded-lg bg-capx-secondary-purple hover:bg-capx-primary-green text-[#F6F6F6] hover:text-capx-dark-bg font-extrabold text-3.5 sm:text-3.5 rounded-lg text-center not-italic leading-[normal] ${isMobile ? "text-md" : "text-lg"}`} label="View Event" />
      </div>
