...
export function useOrganization(token?: string, specificOrgId?: number) {
  ...
  const fetchOrganizations = useCallback(async (token: string, orgIds: number[]) => {
    try {
  ...
  return {
    ...
    updateOrganization: async (data: Partial<Organization>) => {
      ...
      try {
        const updatedOrg = await organizationProfileService.updateOrganizationProfile(
          token,
          specificOrgId,
          data
        );
        setOrganizations(prev => prev.map(org => (org.id === specificOrgId ? updatedOrg : org)));
        return updatedOrg;
      } catch (error) {
