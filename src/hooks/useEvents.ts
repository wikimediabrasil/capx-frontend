    ...
    return eventsToFilter.filter((event) => {
      ...
      if (currentFilters.capacities && currentFilters.capacities.length > 0) {
        ...
        if (
          ...
          const filterCapacityCodes = currentFilters.capacities.map((cap) =>
            Number(cap.code)
          );
      ...
      if (
        ...
        currentFilters.locationType !== EventLocationType.All
      ) {
      ...
      if (currentFilters.organizationId) {
        passedOrganizationFilter =
          event.organization === currentFilters.organizationId;
      }
    ...
    return eventsToFilter.filter((event) => {
      ...
      if (currentFilters.capacities && currentFilters.capacities.length > 0) {
        if (
          ...
          const filterCapacityCodes = currentFilters.capacities.map(
            (cap) => cap.code
          );
      ...
      return (
        passedCapacitiesFilter && passedOrganizationFilter && passedDateFilter
      );
    });
  };
  ...
  useEffect(() => {
    if (allEvents.length > 0 && filters) {
      try {
        ...
        setError(
          err instanceof Error ? err : new Error("Error filtering events")
        );
      }
    }
