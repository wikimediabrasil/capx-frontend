...
async function fetchWikimediaPageData(url: string): Promise<{
  ...
  try {
    ...
    const [contentData, wikidataData, infoboxData] = await Promise.allSettled([
      // 1. Get page content
      fetch(`https://${domain}/w/api.php?action=query&format=json&titles=${encodeURIComponent(cleanTitle)}&prop=extracts&explaintext=true&exsectionformat=plain&origin=*`),
      
      // 2. Get Wikidata QID if available
      fetch(`https://${domain}/w/api.php?action=query&format=json&titles=${encodeURIComponent(cleanTitle)}&prop=pageprops&origin=*`),
      
      // 3. Get page with wikitext to extract infobox
      fetch(`https://${domain}/w/api.php?action=query&format=json&titles=${encodeURIComponent(cleanTitle)}&prop=revisions&rvprop=content&origin=*`)
    ]);
...
async function fetchLearnWikiPageContent(url: string): Promise<{ title?: string; description?: string; dates?: string } | null> {
  try {
