...
function languageCodesMatch(code1: string, code2: string): boolean {
  ...
  if ((normalized1 === 'ptbr' || normalized1 === 'pt-br') && 
...
export function getOrganizationDisplayName(
  ...
  if (!userLanguageTranslation && normalizedUserLang) {
    const baseLang = normalizedUserLang.split('-')[0] || normalizedUserLang.split('_')[0] || normalizedUserLang;
  ...
  const englishTranslation = translations.find(t =>
    normalizeLanguageCode(t.language_code) === 'en'
  );
