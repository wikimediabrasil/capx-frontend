...
export default function Banner({ image, title, alt, customClass = {} }: BannerProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex items-center justify-center h-full px-4 md:px-16 py-6 text-center ${
            title ? 'flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-48' : 'flex-col'
          }`}
        >
          <Image
            src={image}
          ...
          {title &&
            (typeof title === 'string' ? (
              <h1
                className={`text-[20px] md:text-[40px] font-[Montserrat] font-bold ${customClass.text ?? 'text-white'}`}
              >
                {title}
              </h1>
            ) : (
