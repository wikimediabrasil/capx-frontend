...
export default function CapacitySelectionModal({
  ...
  useEffect(() => {
    if (rootCapacitiesData && rootCapacitiesData.length > 0) {
      ...
      const processedCapacities = rootCapacitiesData.map(capacity => {
        ...
        if (typeof capacity.color === 'number' || /^\d+$/.test(String(capacity.color))) {
  ...
  useEffect(() => {
    if (childCapacitiesData && childCapacitiesData.length > 0 && expandingCapacityId) {
  ...
  }, [isOpen, session?.user?.token, preloadCapacities, isLoadingRoots, queryClient]);
  ...
  const handleConfirm = () => {
    if (selectedCapacity) {
      const capacityToReturn: Capacity = {
        ...selectedCapacity,
        skill_type:
          selectedPath.length > 0 ? selectedPath[selectedPath.length - 1] : selectedCapacity.code,
      };
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="flex w-full justify-between gap-2 mt-4">
            ...
            />
            <BaseButton
              label={pageContent['capacity-selection-modal-select-capacity-button']}
