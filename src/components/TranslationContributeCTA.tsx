...
export function TranslationContributeCTA({
  ...
  const getMetabaseContributionUrl = (code: number, mbCode?: string): string => {
    ...
    if (codeStr.startsWith('106')) categoryId = 'Q78'; // technology
    else if (codeStr.startsWith('10')) categoryId = 'Q72'; // organizational
    else if (codeStr.startsWith('36')) categoryId = 'Q73'; // communication
    else if (codeStr.startsWith('50')) categoryId = 'Q74'; // learning
    else if (codeStr.startsWith('56')) categoryId = 'Q75'; // community
    else if (codeStr.startsWith('65')) categoryId = 'Q76'; // social
    else if (codeStr.startsWith('74')) categoryId = 'Q77'; // strategic
  ...
  const helpText = pageContent['translation-help-text'] ||
    ...
    <div className="relative inline-block ml-1">
      {isMobile ? (
        ...
        <div className="group relative inline-block">
          <div className={`inline-flex items-center justify-center w-4 h-4 rounded-full transition-colors cursor-help ${
            darkMode
              ? 'hover:bg-blue-800/50 text-blue-300'
              : 'hover:bg-blue-200 text-blue-600'
          }`}>
          ...
          {/* Desktop tooltip */}
          <div className={`absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-64 p-3 rounded-lg shadow-lg border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 ${
            darkMode
              ? 'bg-gray-800 border-gray-600 text-gray-200'
              : 'bg-white border-gray-300 text-gray-700'
          }`}>
  ...
  if (compact) {
    return (
      <div className={`flex flex-col sm:flex-row sm:items-center gap-2 mt-3 p-2 rounded-md ${
        darkMode
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          {isMobile ? (
            ...
            <div className="flex items-start gap-1">
              <span className={`text-capx-text-xs flex-1 ${
                darkMode ? 'text-blue-200' : 'text-capx-dark-box-bg'
              }`}>
                {pageContent['translation-contribute-compact'] || 'Don\'t see this capacity in your selected language? Help us translate it on Metabase!'}
              </span>
              <HelpIcon />
            </div>
          ) : (
            // Desktop: Text with inline help icon
            <div className={`text-capx-text-xs ${
              darkMode ? 'text-blue-200' : 'text-capx-dark-box-bg'
            }`}>
              {pageContent['translation-contribute-compact'] || 'Don\'t see this capacity in your selected language? Help us translate it on Metabase!'}
              <HelpIcon />
        ...
        >
          <span className={`capx-text-xs ${
            darkMode ? 'text-blue-300' : 'text-blue-600'
          }`}>{pageContent['translation-contribute-link'] || 'Contribute'}</span>
      ...
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0 mt-0.5">
          <div className={`p-1.5 rounded-full ${
            darkMode ? 'bg-capx-dark-box-bg' : 'bg-blue-100'
          }`}>
            <Image
              src={LanguageIcon}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className={`px-2 py-0.5 rounded-full text-xs font-medium ${
              darkMode
                ? 'bg-capx-dark-box-bg text-blue-200'
